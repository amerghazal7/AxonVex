...
| C33 | MEDIUM | **FIXED.** `start()`/`stop()` on both transports were check-then-act on `running_`, so two concurrent `stop()` calls could both pass the guard and both reach `worker_.join()` — joining an already-joined thread is UB. A `lifecycleMutex_` now serialises the whole lifecycle, making start/stop mutually exclusive and idempotent. Regression test `ConcurrentStopIsSafe` (4 threads racing stop, 20 rounds). | `tcpClient.hpp`, `udpSocket.hpp` (fixed) |
