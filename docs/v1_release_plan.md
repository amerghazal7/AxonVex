...
| C32 | MEDIUM | Port callback registration is not thread-safe against dispatch. `OutputPort::outputCallback_`, `AsyncOutputPort::outputCallback_`, `InputPort::dataCallback_`/`validationCallback_` and the `AsyncInputPort` equivalents are `std::function` members read unlocked on the write/read path and written unlocked by their `setXCallback` setters — the same "read without the lock that guards a mutated member" shape as C30, which fixed only `connectedPorts_`. Destroying or replacing a `std::function` while another thread is invoking it is a use-after-free, not merely a torn read. No test or sanitizer run catches it today because every caller registers callbacks before traffic starts. Raised by cpp-reviewer during the C30 review. Fix needs a decision: guard with a mutex (and then snapshot-then-dispatch, since these *are* the user callbacks), or declare registration setup-only and enforce it. | `ports.hpp` (all four port classes) |
| C33 | MEDIUM | `TcpClient::stop()` and `UdpSocket::stop()` check-then-act on `running_`: `if (!running_.load()) return; running_.store(false);`. Two concurrent `stop()` calls can both pass the guard and both reach `worker_.join()` on the same `std::thread` — joining an already-joined thread is UB. Same shape in `start()`. Pre-existing, untouched by C24, and not covered by `UdpConcurrentSendAndStopIsRaceFree` (which races `send` against `stop`, not `stop` against `stop`). Fix is a compare_exchange on `running_` so exactly one caller performs the teardown. Raised by cpp-reviewer during the C24 review. | `tcpClient.hpp`, `udpSocket.hpp` (`start`/`stop`) |
...
| C24 | LOW | **Race half FIXED**, rest open. Fixed: the TCP/UDP fd close race. `stop()` closed the socket while the receive thread was blocked in `recv`/`recvfrom` on it and `send()` read `sock_` with no guard — TSan reported races on `sock_`, on the descriptors themselves, and on the plain-`uint64_t` `ProtocolStatistics` counters shared by the send and receive threads. Closing an fd another thread is blocked on is worse than a torn read: the number is immediately reusable, so the blocked call can end up on a stranger's descriptor. Fix: `SO_RCVTIMEO` (100 ms) bounds the receive block so the loop exits on `running_` by itself; `stop()` joins *before* closing (TCP additionally calls `shutdown()` first, which wakes a blocked `recv` without invalidating the fd); `send()` holds a new `sockMutex_` across the syscall; UDP peer state moves under `peerMutex_`; counters move to a new `AtomicProtocolStatistics` with a plain-struct `snapshot()` so callers are unaffected. Regression test `UdpConcurrentSendAndStopIsRaceFree` reproduced all five races against the pre-fix headers. Un-quarantined — no known race remains in the sanitizer gate. **Still open:** IPv4-only, `inet_pton`-only (no DNS, "localhost" fails) — belongs to the §6 `axonvex_net` production pass; and non-Linux builds still silently no-op and return `true`, which is the banned silent-failure class. | `udpSocket.hpp`, `tcpClient.hpp` (race fixed); DNS/IPv6/non-Linux stubs open |
