...
| C29 | HIGH | Ports' pooled read path is a use-after-free, not a missing atomic: `read()` dereferences `pooledData_` (`ports.hpp:461-463`) while `writeData()` swaps in a new block and immediately returns the old one to the pool (`ports.hpp:508-511`), so a later write recycles and reconstructs the block a reader is still inside. TSan-confirmed via `MemoryPoolPortsTest.ConcurrentAccessPatterns`; same shape in `InputPort`, `OutputPort`, `AsyncInputPort`. Quarantined from the TSan CI gate until fixed. **Decided fix (2026-08-03, user): delete the pooled fast path** — the mutex path in the same functions is already correct, and lock-free reads need real reclamation (hazard pointers/epoch) that no benchmark yet justifies. Scope: drop `memoryPool_`/`pooledData_`/`poolSize_` and the pooled branches from all four port classes (~40 sites), remove the pure-virtual `setMemoryPoolSize`/`getMemoryPoolSize` from `BasePort` rather than leaving silent no-ops, rewrite `tests/core/memoryPoolPortsTest.cpp` as mutex-path thread-safety tests (keep the concurrency coverage), delete `examples/memory_pool_ports_simple_example.cpp`. `MemoryPool` itself stays — it has other users. | `ports.hpp:461,505,668,833,888`, `memoryPool.hpp:225` |
