...
| C34 | HIGH | Both transports invoke user callbacks while holding `cbMutex_`: `recvLoop` dispatches messages under it, and `reportError` dispatches error handlers under it. Same class as C12/C18 (never run user code under a lock), in `axonvex_interfaces` rather than core. This is also what made C33's self-join reachable — callbacks run on the worker thread, so a callback that calls `stop()` re-enters the lifecycle. The self-join is now guarded, but the underlying lock-around-user-code remains: a callback that takes its own lock in the opposite order, or that calls back into the transport's registration API, still deadlocks on `cbMutex_`. Fix is snapshot-then-dispatch, as done for `SafetyManager` in C12. Raised by cpp-reviewer during the C24/C32/C33 review. | `tcpClient.hpp`, `udpSocket.hpp` (`recvLoop`, `reportError`) |
