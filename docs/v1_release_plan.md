...
| C30 | HIGH | `AsyncOutputPort<T>::write()` reads `connectedPorts_.empty()` with no lock held, three lines before taking `connectionMutex_` to iterate the same vector — while `connect`/`disconnect`/`disconnectAll` mutate it under that mutex. Unsynchronized read of a `std::vector` being reallocated: a TOCTOU on the emptiness check and a genuine data race. Pre-dates the port system's current form (present since `d345805`); found independently during the C29 review and by inspection. Fix is the one-line move of the check inside the existing lock — no new mutex, no ordering argument needed. | `ports.hpp:759` |
| C31 | MEDIUM | `BasePort::setThreadSafe` selects between a locked and an unlocked access discipline via an atomic flag, so toggling it while producers/consumers are live races on the payload itself: a writer that observed `true` takes the mutex path concurrently with a reader that observed `false` taking the unlocked path on the same `data_`/`currentData_`. The atomic makes the flag safe, not the switch. Today nothing toggles it post-startup (`interfaceUnits.hpp:24,112` call it at construction; `ProcessingUnit::setPortsThreadSafe` has no callers), so this is latent — but it is public API with no compile-time or test guard. Documented as a precondition on `BasePort::setThreadSafe` as an interim measure; the real fix is to make thread-safety a construction-time property of the port rather than a settable one. | `ports.hpp:93`, `processingUnit.cpp:142-156` |
