...
struct LogStatistics {
    ...
    uint64_t getMessagesLogged() const noexcept {
        return messages_logged.load();
    }
    uint64_t getMessagesDropped() const noexcept {
        return messages_dropped.load();
    }
    uint64_t getMessagesForLevel(LogLevel level) const noexcept {
        return messages_by_level[static_cast<size_t>(level)].load();
    }
    uint64_t getOutputFailures() const noexcept {
        return output_failures.load();
    }
    uint64_t getQueueOverflows() const noexcept {
        return queue_overflows.load();
    }
    uint64_t getTotalProcessingTimeNs() const noexcept {
        return total_processing_time_ns.load();
    }
    uint64_t getPeakQueueSize() const noexcept {
        return peak_queue_size.load();
    }
...
struct LogMessage {
    ...
    LogMessage(LogLevel lvl, const std::string& cat, const std::string& msg,
        ...
        : level(lvl), timestamp(std::chrono::high_resolution_clock::now()),
          thread_id(std::this_thread::get_id()), category(cat), message(msg), file(f), line(l),
          function(func) {}
};
...
class Logger {
  public:
     ...
     */
    explicit Logger(size_t queue_size = DEFAULT_QUEUE_SIZE, size_t pool_size = DEFAULT_POOL_SIZE);
    ...
    void logf(LogLevel level, const std::string& category, const std::string& format,
              Args&&... args);
    ...
    void debug(const std::string& category, const std::string& message,
               const std::string& file = "", int line = 0, const std::string& function = "");
    void info(const std::string& category, const std::string& message, const std::string& file = "",
...
template <typename... Args>
inline void Logger::logf(LogLevel level, const std::string& category, const std::string& format,
                         Args&&... args) {
    if (level < min_level_.load(relaxed)) {
        return;
    }
...
inline std::string Logger::getPerformanceMetrics() const {
    ...
    metrics += "  Average processing time: " + std::to_string(stats.getAverageProcessingTimeNs()) +
               " ns\n";
...
inline void Logger::processMessage(LogMessage* message) {
    ...
    for (auto& output : outputs_) {
        if (output) {
            ...
            } catch (...) { stats_.output_failures.fetch_add(1, relaxed); }
        }
    }
}
