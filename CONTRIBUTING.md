...
- **Formatting.** Run `clang-format` with the repo's `.clang-format`; follow [docs/STYLE_GUIDE.md](docs/STYLE_GUIDE.md). CI enforces clang-format **18.1.8** and clang-tidy **18.1.8** exactly (see `.github/workflows/ci.yml`) — different majors format/lint some constructs differently, so install the same versions locally: `pip install "clang-format==18.1.8" "clang-tidy==18.1.8"`.
